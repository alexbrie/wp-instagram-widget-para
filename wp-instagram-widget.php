<?php
/*
Plugin Name: WP Instagram Widget Paragraphed
Plugin URI: https://github.com/alexbrie/wp-instagram-widget-para
Description: A WordPress widget for showing your latest Instagram photos. Slight tweak of https://github.com/scottsweb/wp-instagram-widget by Scott Evans
Version: 1.0
Author: Alex Brie
Author URI: http://alexbrie.net
Text Domain: wp-instagram-widget-para
Domain Path: /assets/languages/
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Copyright © 2017 Alex Brie
...
class null_instagram_widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
			...
			if ( is_wp_error( $media_array ) ) {
				...
				foreach ( $media_array as $item ) {
					...
					if ( locate_template( $template_part ) != '' ) {
						...
						echo '<p><a href="'. esc_url( $item['link'] ) .'" target="'. esc_attr( $target ) .'"  class="'. esc_attr( $aclass ) .'"><img src="'. esc_url( $item[$size] ) .'"  alt="'. esc_attr( $item['description'] ) .'" title="'. esc_attr( $item['description'] ).'"  class="'. esc_attr( $imgclass ) .'"/></a></p>';
					}
				}
				?><?php
			}
		...
		if ( $text != '' ) {
			?><p class="<?php echo esc_attr( $linkclass ); ?>"><?php echo wp_kses_post( $text ); ?></p><?php
		}
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'text' ) ); ?>"><?php esc_html_e( 'Text', 'wp-instagram-widget' ); ?>: <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'text' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'text' ) ); ?>" type="text" value="<?php echo esc_attr( $text ); ?>" /></label></p>
